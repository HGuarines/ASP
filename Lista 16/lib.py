...
def rmg_feixe(dicionario, inicio, posicao_nc):
    ...
    for i in range(nc):
        for j in range(nc):
            if i != j:
                A = A * distancia(Xc[i+posicao_nc], Yc[i+posicao_nc], Xc[j+posicao_nc], Yc[j+posicao_nc])
    RMGF = (A*(1 / (nc*2)))
    ...
    for i in range(len(Xc)):
        for j in range(len(Xc)):
            ...
            if j == i:
                ...
                ZLT[i][j] = r[i] + 1j * 4 * np.pi * 1e-4 * 60.0 * np.log(d / RWG[i])
...
def matrix_Alt(raio_cond, dados):
    """
    Calcula a matriz A para linhas de transmissão.
    ...
    --------
    ALT : matrix
        Matriz de coeficientes de campo
    """
    import numpy as np
    Xc = dados['X']
    Yc = dados['Y']

    ALT = np.zeros((len(Xc), len(Xc)), dtype=complex)
    ...
    for i in range(len(Xc)):
        for j in range(len(Xc)):
            ...
            if j == i:
                # Elementos da diagonal
                d = distancia(Xc[i], Yc[i], Xc[i], -Yc[i])
                ALT[i][j] = 1j * (2 * 8.85 * np.pi * 1e-9)**-1 * np.log(d / raio_cond[i])

            else:
                # Elementos fora da diagonal
                di = distancia(Xc[i], Yc[i], Xc[j], Yc[j])
                dii = distancia(Xc[i], Yc[i], Xc[j], -Yc[j])
                ALT[i][j] = 1j * (2 * 8.85 * np.pi * 1e-9)**-1 * np.log(dii / di)

    titulo = '    Matrix A da LT'
    print(titulo)
    ...
    for i in range(len(Xc)):
        for j in range(len(Xc)):
            ...
            B = j + 1
            if X >= 0.0:
                print('    ALT(%id,%id) - %8.4f + %8.4fj ' % (A, B, R, X))
            else:
                print('    ALT(%id,%id) - %8.4f - %8.4fj ' % (A, B, R, abs(X)))
...
def matrix_Zlt_sem_CI(r, dados):
    ...
    for i in range(len(Xc)):
        for j in range(len(Xc)):
            ...
            if j == i:
                # Elementos da diagonal
                ZLT[i][j] = r[i] + 1j * 4 * np.pi * 1e-4 * 60.0 * np.log(1 / RWG[i])
...
def matrix_Alt_sem_CI(raio_cond, dados):
    """
    Calcula a matriz A para linhas de transmissão.
    ...
    for i in range(len(Xc)):
        for j in range(len(Xc)):
            ...
            if j == i:
                # Elementos da diagonal
                ALT[i][j] = 1j * (2 * 8.85 * np.pi * 1e-9)**-1 * np.log(1 / raio_cond[i])

            else:
                # Elementos fora da diagonal
                di = distancia(Xc[i], Yc[i], Xc[j], Yc[j])
                ALT[i][j] = 1j * (2 * 8.85 * np.pi * 1e-9)**-1 * np.log(1 / di)

    titulo = '    Matrix A da LT sem CI'
    print(titulo)
...
def criar_submatrizes(matriz):
    ...
    if len(matriz) == 8:
        ...
        # Matriz 3x3 - 6 primeiras linhas e 6 primeiras colunas
        matriz1 = []
        for i in range(6):
            linha = []
            for j in range(6):
        ...
        # Matriz 6x2 - 6 primeiras linhas e 2 últimas colunas
        matriz2 = []
        for i in range(6):
            linha = []
            for j in range(6, 8):  # colunas 7 e 8 
                linha.append(matriz[i][j])
            matriz2.append(linha)
        
        # Matriz 2x6 - 2 últimas linhas e 6 primeiras colunas
        matriz3 = []
        for i in range(6, 8):  # linhas 7 e 8 
            linha = []
            for j in range(6):
                linha.append(matriz[i][j])
...
def matriz_impedancia_reduzida(ZLT):
    ...
    if len(Zabccp) == 3:
        ...
        Zs1, Za1, Zb1, Zc1 = Zserv(Zabccp)
        
        return Zs1, Za1, Zb1, Zc1, Zabccp  
    
    else:
        
        Zs1, Za1, Zb1, Zc1, Zs2, Za2, Zb2, Zc2 = Zserv(Zabccp)
        
        return Zs1, Za1, Zb1, Zc1, Zs2, Za2, Zb2, Zc2, Zabccp
...
def Zserv(ZLT):
    ...
    if len(ZLT) == 3:
        ...
        Za1 = ZLT[0][0] - (1/2) * (ZLT[0][1] + ZLT[0][2])
        Zb1 = ZLT[1][1] - (1/2) * (ZLT[1][0] + ZLT[1][2])
        Zc1 = ZLT[2][2] - (1/2) * (ZLT[2][0] + ZLT[2][1])
        
        # Impedância de serviço média LT1
        Zs1 = (Za1 + Zb1 + Zc1) / 3
        
        Za2 = ZLT[3][3] - (1/2) * (ZLT[3][4] + ZLT[3][5])
        Zb2 = ZLT[4][4] - (1/2) * (ZLT[4][3] + ZLT[4][5])
        Zc2 = ZLT[5][5] - (1/2) * (ZLT[5][3] + ZLT[5][4])   
        
        # Impedância de serviço média LT2
        Zs2 = (Za2 + Zb2 + Zc2) / 3

    return Zs1, Za1, Zb1, Zc1, Zs2, Za2, Zb2, Zc2
...
def Cserv(CLT):
    ...
    # Cálculo das impedâncias de sequência
    if len(CLT) == 3:
        Ca = CLT[0][0] - (1/2) * (CLT[0][1] + CLT[0][2])
        Cb = CLT[1][1] - (1/2) * (CLT[1][0] + CLT[1][2])
        Cc = CLT[2][2] - (1/2) * (CLT[2][0] + CLT[2][1])
        # Impedância de serviço média
        Cs = (Ca + Cb + Cc) / 3
        ...
        Ca1 = CLT[0][0] - (1/2) * (CLT[0][1] + CLT[0][2])
        Cb1 = CLT[1][1] - (1/2) * (CLT[1][0] + CLT[1][2])
        Cc1 = CLT[2][2] - (1/2) * (CLT[2][0] + CLT[2][1])
        
        # Impedância de serviço média LT1
        Cs1 = (Ca1 + Cb1 + Cc1) / 3
        
        Ca2 = CLT[3][3] - (1/2) * (CLT[3][4] + CLT[3][5])
        Cb2 = CLT[4][4] - (1/2) * (CLT[4][3] + CLT[4][5])
        Cc2 = CLT[5][5] - (1/2) * (CLT[5][3] + CLT[5][4])   
        
        # Impedância de serviço média LT2
        Cs2 = (Ca2 + Cb2 + Cc2) / 3

    return Cs1, Ca1, Cb1, Cc1, Cs2, Ca2, Cb2, Cc2
